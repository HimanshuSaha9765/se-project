...
use App\Models\User;
use App\Services\EmployeeClientService;
...
class EmployeeClientController extends Controller
{
    ...
    public function __construct(EmployeeClientService $employeeClientService)
    {
        $this->employeeClientService = $employeeClientService;
    }
    var $consumer_number, $compact_data;
    public function manage_client()
    {
        $this->compact_data = $this->employeeClientService->manage_client();
        return view("employee.Client.manage_client", $this->compact_data);
    }
    public function add_client()
    {
        $this->compact_data = $this->employeeClientService->add_client();
        $User_data = references::query()->where('status', '!=', 'deleted')->get();
        return view("employee.Client.add_client",  $this->compact_data);
    }
    public function insert_client(Request $request)
    {
        ...
        $this->compact_data = $this->employeeClientService->insert_client($request);
        ...
        if ($this->compact_data == "true") {
            ...
            return redirect()->route('employee.manage_client');
        } else {
            session()->flash('error', 'Error in adding client added.');
            return redirect()->route('employee.add_user');
        }
        // } 
        // catch (Exception $th) {
        //     dd($th->getMessage());
    ...
    {
        $this->compact_data = $this->employeeClientService->edit_client($id);
        return view('employee.Client.update_client', $this->compact_data);
    }
    public function update_client(Request $request)
    {
        $this->compact_data = $this->employeeClientService->update_client($request);
    ...
    {
        $this->compact_data = $this->employeeClientService->delete_client($id);
    ...
    {
        $this->compact_data = $this->employeeClientService->update_client_status($id, $status);
    ...
    {
        $this->compact_data = $this->employeeClientService->update_client_permision($id,$permision);
    ...
    {
        $this->compact_data = $this->employeeClientService->client_details($consumer_number);
        return view('employee.Client.client_details', $this->compact_data);
    }
    //* Client Tracking Module
    public function add_client_tracking()
    {
        $this->compact_data = $this->employeeClientService->add_client_tracking();
        return view('employee.Client.add_client_tracking', $this->compact_data);
    }


    public function insert_client_tracking(Request $insert_client_tracking)
    {
        $this->compact_data = $this->employeeClientService->insert_client_tracking($insert_client_tracking);
        return redirect()->route('employee.add_client_tracking', ['authUser' => $this->compact_data]);
    }
    
    public function update_client_tracking(Request $request)
    {
        $this->compact_data = $this->employeeClientService->update_client_tracking($request);
        return redirect()->route('employee.add_client_tracking', ['authUser' => $this->compact_data]);
    }

    public function add_client_document()
    {
        $this->compact_data = $this->employeeClientService->add_client_document();
        return view("employee.Client.add_client_document", $this->compact_data);
    }
    public function insert_client_document(Request $request)
    {
        $this->compact_data = $this->employeeClientService->insert_client_document($request);
        if ($this->compact_data == "false") {
            session()->flash('error', 'Error in Adding Client Document Added.');
            return redirect()->route('employee.add_client_document');
        } else {
            session()->flash('success', 'Client Document Added Successfully.');
            return redirect()->route('employee.client_details', ['consumer_number' => $request->consumer_number]);
        }
    ...
    {
        $this->compact_data = $this->employeeClientService->edit_client_document($consumer_number);
        return view('employee.Client.update_client_document',$this->compact_data);
    }
    public function update_client_document(Request $request)
    {
        $this->compact_data = $this->employeeClientService->update_client_document($request);
        session()->flash('success', 'Document Update Successfully.');
        return redirect()->route('employee.download_document_and_update', ['authUser' => $this->compact_data]);
    }
    public function download_document_and_update()
    {
        $this->compact_data = $this->employeeClientService->download_document_and_update();
        return view("employee.Client.download_document_and_update", $this->compact_data);
    }
}
