...
class ClientService implements ClientRepo
{
    ...
    {
        $User_data = references::query()->whereRaw('status != ?', ['inactive'])->get();
    ...
    public function insert_client($request)
    {
        ...
        $client->reference_by = $references_data->name . ' - ' . $references_data->city_name;
        $client->structure_image = $Image;
        $client->address = $request->address;

        $this->compact_data = $client;
        return $this->compact_data;
    }
    ...
    public function update_client($request)
    {
        try {
            ...
            $client->update([
                ...
                'reference_by' => $references_data->name . ' - ' . $references_data->city_name,
    ...
    public function delete_client($id)
    {
        ...
        return !empty($this->compact_data['Client']) ? "false" : "true";
    }
    public function update_client_status($id, $status)
    {
        return Client::query()->whereRaw('id = ?', [$id])->update(['status' => $status]) ? "true" : "false";
    }
    public function update_client_permision($id, $permision)
    {
        return Client::query()->whereRaw('id = ?', [$id])->update(['process_of_client' => $permision]) ? "true" : "false";
    }
    public function client_details($consumer_number)
    {
        try {
            ...
            // dd($Client_tracking_Data->client_tracking->name);
    ...
    public function insert_client_tracking($request)
    {
        ...
        if (!$Client_tracking_Data) {
            ...
            if ($request->application_number_1 || $request->appication_1 || $request->amount_1) {
                ...
                $Track1->update([
                    ...
                    'appication_1' => $request->appication_1,
        ...
        return $this->compact_data['authUser'] = encrypt($request->consumer_number);
        // return redirect()->route('admin.add_client_tracking', ['authUser' => ]);
    }
    ...
    public function update_client_tracking($request)
    {
        ...
        if ($request->inverter_image7 || $request->serial_number_image7 || $request->serial_number7) {
            ...
            if ($request->hasFile('inverter_image7')) {
                ...
                $inverter_image7 = $this->verifyAndUpload_client_tracking($request, 'inverter_image7');
                $Track7->where('consumer_number', $request->consumer_number)->update(['inverter_image7' => $inverter_image7]);
            } else {
            ...
            if ($request->hasFile('serial_number_image7')) {
                ...
                $serial_number_image7 = $this->verifyAndUpload_client_tracking($request, 'serial_number_image7');
                $Track7->where('consumer_number', $request->consumer_number)->update(['serial_number_image7' => $serial_number_image7]);
            } else {
        ...
        if ($request->perform_8 || $request->form_16_8) {
            ...
            if ($request->hasFile('perform_8')) {
                ...
                $perform_8 = $this->verifyAndUpload_client_tracking($request, 'perform_8');
                $Track8->where('consumer_number', $request->consumer_number)->update(['perform_8' => $perform_8]);
            } else {
            ...
            if ($request->hasFile('form_16_8')) {
                ...
                $form_16_8 = $this->verifyAndUpload_client_tracking($request, 'form_16_8');
                $Track8->where('consumer_number', $request->consumer_number)->update(['form_16_8' => $form_16_8]);
            } else {
        ...
        if ($request->jr_form_9 || $request->subsidy_clamp_9 || $request->amount_9 || $request->description_9 || $request->recived_9 || $request->pdf_9) {
            $Track9 = client_tracking::query()->where('consumer_number', $request->consumer_number)->first();
            ...
            if ($request->hasFile('jr_form_9')) {
                ...
                $jr_form_9 = $this->verifyAndUpload_client_tracking($request, 'jr_form_9');
                $Track9->where('consumer_number', $request->consumer_number)->update(['jr_form_9' => $jr_form_9]);
            } else {
            ...
            if ($request->hasFile('pdf_9')) {
                ...
                $pdf_9 = $this->verifyAndUpload_client_tracking($request, 'pdf_9');
                $Track9->where('consumer_number', $request->consumer_number)->update(['pdf_9' => $pdf_9]);
            } else {
                $pdf_9 = $Track9->pdf_9;
            }
    ...
    public function add_client_document()
    {
        try {
            ...
            return $this->compact_data = $e->getMessage();
        }
    }
    ...
    public function insert_client_document($request)
    {
        try {
            ...
            $client_document->variation_amount = $request->final_confirm_amount;
            ...
            Client::where('consumer_number', $request->consumer_number)
                ->update([
                        'quotation_amount' => $request->final_confirm_amount,
                    ]);

            // dd($client_document);
            if ($client_document->save()) {

                //TODO : Payment Table Insert Data 
                $randomKeySha1 = sha1(uniqid());
                $info_id_payment = 'Payment-' . $randomKeySha1;

                $email = session()->get('admin');
                $data = User::query()->whereRaw('email = ?', [$email])->first();

                $info = new Log_Infos();
                $info->table_id = $info_id_payment;
                $info->created_ip = $request->ip();
                $info->created_name = $data->name;
                $info->created_email = $email;
                $info->created_date = Carbon::now('Asia/Kolkata')->format('Y-m-d h:i:s A');
                ...
                $client_payment->info_id = $info_id_payment;
                $client_payment->consumer_number = $request->consumer_number;
                $client_payment->payment_date = $currentDate;
                $client_payment->payment_mode = 'cash';
                $client_payment->final_confirm_amount = $request->final_confirm_amount;
                $client_payment->recieved_amount = $request->deposit ?? '0';
                $client_payment->due_amount = $request->due_amount ?? '0';
                $client_payment->total_amount = '0';
                // dd($client_payment);
                $client_payment->save();

                session()->flash('success', 'Client Document Added Successfully.');
                return $this->compact_data['consumer_number'] = $request->consumer_number;
                // return redirect()->route('admin.client_details', ['consumer_number' => $request->consumer_number]);
            } else {
    ...
    public function edit_client_document($consumer_number)
    {
        try {
            ...
            return $this->compact_data['message'] = $e->getMessage();
        }
    }
    ...
    public function update_client_document($request)
    {
        ...
        // dd($request->hasFile('adharcard_image'));
        // * Start Log Update
        ...
        $f_amount =  $request->final_confirm_amount;
        // dd($f_amount);
        $document_data->update([
            ...
            'final_confirm_amount' => $f_amount,
            'variation_amount' => $f_amount,
            'deposit' => $request->deposit,
            'due_amount' => $request->due_amount,
        ]);
        // dd($document_data->variation_amount);

        Client::where('consumer_number', $request->consumer_number)
            ->update([
                    'quotation_amount' => $f_amount,
                ]);

        return $this->compact_data['authUser'] = encrypt($request->consumer_number);
        // return redirect()->route('admin.download_document_and_update', ['authUser' => encrypt($request->consumer_number)]);
    }
