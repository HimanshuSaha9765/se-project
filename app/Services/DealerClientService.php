...
use App\Models\User;
use App\Repository\DealerClientRepo;
...
class DealerClientService implements DealerClientRepo
{
    ...
    public function manage_client()
    {
        ...
        $clients = Client::query()->orderBy('id', 'desc')->where('status', '!=', 'deleted')->where('user_email_id', $email)->get();
    ...
    {
        $email = session()->get('dealer');
        $User_data = references::query()->where('email_assign',$email)->first();
        return $this->compact_data['User_data'] = $User_data;
    }
    ...
    public function insert_client($request)
    {
        ...
        $client->structure_image = $Image ?? 'default.jpg';
    ...
    public function update_client($request)
    {
        try {
            ...
            return $this->compact_data = "true";
            // return redirect()->route('dealer.manage_client');
        } catch (Exception $e) {
            // dd($e->getMessage());
            session()->flash('error', 'Error in updating client: ' . $e->getMessage());
            return $this->compact_data = $request->id;
            // return redirect()->route('dealer.edit_client', ['id' => $request->id]);
        }
    }
    ...
    public function add_client_tracking()
    {
        try {
            ...
            // dd($Client_Document);
            // dd($Client_tracking_Data->consumer_number);
            $this->compact_data['consumer_number'] = $consumer_number;
            $this->compact_data['Client_tracking_Data'] = $Client_tracking_Data;
    ...
    public function insert_client_document($request)
    {
        try {
            ...
            if ($client_document->save()) {
                return $this->compact_data = $request->consumer_number;
            } else {
                return $this->compact_data = "true";
            }
        } catch (Exception $e) {
    ...
    public function download_document_and_update()
    {
        try {
            ...
            // dd($consumer_number);
            $Client_Document = Client_Document::query()->whereRaw('consumer_number = ?', [$consumer_number])->first();
            $payment = Payment::query()->whereRaw('consumer_number = ?', [$consumer_number]);
            $this->compact_data['Client_Document'] = $Client_Document;
            $this->compact_data['payment'] = $payment;
            // dd($Client_Document);
            // $client = $Client_Document->Clients;
            return $this->compact_data;
        } catch (Exception $e) {
            return $this->compact_data = $e->getMessage();
        }
