    ...
    <div class="x_panel">
        <div class="x_content">
            <div style="display: flex; justify-content: flex-end;">
                <button type="button" class="btn btn-outline-dark" data-toggle="modal" data-target="#DealerCityModal"
                    data-whatever="@mdo">
            ...
            @include('admin.Dealer City.add_dealer_city_modal')
            @include('admin.Dealer City.edit_dealer_city_modal')
                ...
                <div class="col-sm-12">
                    <div class="card-box table-responsive" style="padding: 1rem;">
                        <table id="referencesTable" class="table table-bordered  table-hover text-nowrap text-center"
                            ...
                            <tbody>
                                @foreach ($Dealer_citys as $Dealer_city)
                                    <tr>
                                        <td>
                                            <div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '.toggle-status', function() {
                var dealerCityId = $(this).data("city-id");
                var status = $(this).prop('checked') ? 'active' : 'inactive';
                var toggleElement = $(this);

                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.update_dealer_city_status', ['id' => ':id', 'status' => ':status']) }}'
                        .replace(':id', dealerCityId)
            ...
            $(document).on('click', '#delete_btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('admin.delete_dealer_city', ['id' => ':id']) }}'
