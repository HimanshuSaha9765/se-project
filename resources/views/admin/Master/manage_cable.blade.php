    ...
    <div class="x_panel">
        <div class="x_content">
            <div style="display: flex; justify-content: flex-end;">
                <button type="button" class="btn btn-outline-dark" data-toggle="modal" data-target="#cableModal"
                    data-whatever="@mdo">
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="cableTableModalLabel">Add Cable</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form id="AddCableForm" action="{{ route('admin.insert_cable') }}" method="POST">
                                @csrf
                                <div class="mb-3">
                                    <label for="cable" class="col-form-label required_input">Type of Cabel</label>
                                    <input type="text" class="form-control" name="cable_type" id="cable-type"
                                        placeholder="Enter Cable Type" required>
                                </div>
                                <div class="mb-3">
                                    <label for="unit" class="col-form-label">Lenght (mm)</label>
                                    <input type="text" class="form-control" name="cable_length" id="cable-length"
                                        placeholder="Enter Cable Length">
                                </div>

                                <div class="mb-3">
                                    <label for="unit" class="col-form-label">Choose Color</label>
                                    <select class="form-control select2" id="cable-color" name="cable_color"
                                        style="width: 100%">
            ...
            <div class="modal fade" id="cableEditModal" tabindex="-1" role="dialog" aria-labelledby="cableModalEditLabel"
                aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="cableTableModalLabel">Edit Cable</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form id="EditCableForm" method="POST" action="{{ route('admin.update_cable') }}">
                            @csrf
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    <label for="cable" class="col-form-label required_input">Type of Cabel</label>
                                    <input type="text" class="form-control" name="cable_type_edit" id="cable-type-edit"
                                        placeholder="Enter Cable Type" required>
                                </div>
                                <div class="mb-3">
                                    <label for="unit" class="col-form-label">Lenght (mm)</label>
                                    <input type="text" class="form-control" name="cable_length_edit"
                                        id="cable-length-edit" placeholder="Enter Cable Length">
                                </div>

                                <div class="mb-3">
                                    <label for="unit" class="col-form-label">Choose Color</label>
                                    <select class="form-control select2" id="cable-color-edit" name="cable_color_edit"
                                        style="width: 100%">
                                        <option value="red">Red</option>
                                        <option value="green">Green</option>
                                        <option value="black">Black</option>
            ...
            {{-- INFO MODAL --}}
            <div class="modal fade bs-example-modal-lg" id="cableInfoModal" tabindex="-1" role="dialog"
                aria-hidden="true">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box table-responsive" style="padding: 1rem;">
                        <table id="cableTable" class="table table-bordered  table-hover text-nowrap text-center"
                            style="width:100%">
                            <thead style="background: white;text-align: center; align-items: center;">
                                <tr>
                                    <th class="text-center">Actions</th>
                                    <th class="text-center">Cable Type</th>
                                    <th class="text-center">Cable Length (mm)</th>
                                    <th class="text-center">Cable Color</th>
                            ...
                            <tbody>
                                @foreach ($Cables as $cable)
                                    <tr>
                                            ...
                                            <div>
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                                    <button class="dropdown-item edit-btn" type="button"
                                                        data-id="{{ $cable->id }}"> <i class="fa fa-edit mx-2"></i>
                                                        Edit</button>
                                                    ...
                                                    <button id="info" class="dropdown-item" type="button"
                                                        data-id="{{ $cable->info_id }}" data-toggle="modal"
                                                        data-target="#cableInfoModal">
                                        ...
                                        <td>{{ $cable->cable_length ?? '-' }}</td>
                                        <td>
                                            <div class="row" style="align-items: center;justify-content: center">
                                                @if ($cable->cable_color === 'red')
                                                    <div style="height: 20px;width: 20px;background-color: red"></div>
                                                @elseif ($cable->cable_color === 'green')
    ...
    <script>
        $(document).ready(function() {
            $('#cableTable thead tr')
                .clone(true)
                .addClass('filters')
                .appendTo('#cableTable thead');

            var table = $('#cableTable').DataTable({
                dom: 'Bfrtip',
                stateSave: true,
                ordering: false,
                pageLength: 0,
            ...
            $('.table').on('click', '.edit-btn', function(e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route('admin.edit_cable', ['id' => ':id']) }}'
                        .replace(':id', id),
                    type: "GET",
                    success: function(data) {
                        $('#id').val(data.id);
                        $('#cable-type-edit').val(data.cable_type);
                        $('#cable-length-edit').val(data.cable_length);
                        $('#cable-color-edit').val(data.cable_color);
                        $('#cable-color-edit').trigger('change');
            ...
            $(document).on('change', '.toggle-status', function() {
                var cableId = $(this).data("cable-id");
                var status = $(this).prop('checked') ? 'active' : 'inactive';
                var toggleElement = $(this);

                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.update_cable_status', ['id' => ':id', 'status' => ':status']) }}'
                        .replace(':id', cableId)
            ...
            $(document).on('click', '#delete_btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('admin.delete_cable', ['id' => ':id']) }}'
                                .replace(':id', cableId),
                            type: 'GET',
                            success: function(response) {
                                // Swal.fire({
