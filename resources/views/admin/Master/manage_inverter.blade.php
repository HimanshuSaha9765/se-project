    ...
    <div class="x_panel">
        <div class="x_content">
            <div style="display: flex; justify-content: flex-end;">
                <button type="button" class="btn btn-outline-dark" data-toggle="modal" data-target="#inverterModal"
                    data-whatever="@mdo">
            ...
            <div class="modal fade" id="inverterModal" tabindex="-1" role="dialog" aria-labelledby="inverterModalLabel"
                aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="inverterTableModalLabel">Add Inverter</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form id="AddInverterForm" method="POST" action="{{ route('admin.insert_inverter') }}">
                                @csrf
                                <div class="form-group">
                                    <label for="recipient-name" class="col-form-label required_input">Inverter Name</label>
                                    <input type="text" class="form-control" id="inverter-name" name="inverter_name"
                                        placeholder="Enter Inverter Name" required>
                                    <span id="err_name" style="color:red">
                                        @error('inverter_name')
                                            {{ $message }}
                                        @enderror
                                    </span>
                                ...
                                <div class="form-group">
                                    <label for="message-text" class="col-form-label">Unit (kw)</label>
                                    <input type="text" class="form-control" id="Unit" name="unit"
                                        placeholder="Enter Unit in (kw)">
                                    <span id="err_unit" style="color:red">
                                        @error('unit')
                                            {{ $message }}
                                        @enderror
                                    </span>
            ...
            <div class="modal fade" id="inverterEditModal" tabindex="-1" role="dialog"
                aria-labelledby="inverterModalEditLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="inverterTableModalLabel">Edit Inverter</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form id="EditInverterForm" method="POST" action="{{ route('admin.update_inverter') }}">
                            @csrf
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label for="recipient-name" class="col-form-label required_input">Inverter Name</label>
                                    <input type="text" class="form-control" id="inverter-name-edit"
                                        name="inverter_name_edit" placeholder="Enter Inverter Name" required>
                                </div>
                                <div class="form-group">
                                    <label for="message-text" class="col-form-label">Unit (kw)</label>
                                    <input type="text" class="form-control" id="Unit-edit" name="unit_edit"
                                        placeholder="Enter Unit in (kw)">
                                </div>
                            </div>
                            <div class="modal-footer">
            ...
            {{-- INFO MODAL --}}
            <div class="modal fade bs-example-modal-lg" id="inverterInfoModal" tabindex="-1" role="dialog"
                aria-hidden="true">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box table-responsive" style="padding: 1rem;">
                        <table id="inverterTable" class="table table-bordered  table-hover text-nowrap text-center"
                            style="width:100%">
                            <thead style="background: white;text-align: center; align-items: center;">
                                <tr>
                                    <th class="text-center">Actions</th>
                                    <th class="text-center">Inverter Name</th>
                                    <th class="text-center">Unit (kw)</th>
                                            ...
                                            <div>
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                                    <button class="dropdown-item edit-btn" type="button"
                                                        data-id="{{ $inverter->id }}"> <i class="fa fa-edit mx-2"></i>
                                                        Edit</button>
                                                    ...
                                                    <button id="info" class="dropdown-item" type="button"
                                                        data-id="{{ $inverter->info_id }}" data-toggle="modal"
                                                        data-target="#inverterInfoModal">
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            $('#inverterTable thead tr')
                .clone(true)
                .addClass('filters')
                .appendTo('#inverterTable thead');

            var table = $('#inverterTable').DataTable({
                dom: 'Bfrtip',
                stateSave: true,
                ordering: false,
                pageLength: 0,
            ...
            $('.table').on('click', '.edit-btn', function(e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route('admin.edit_inverter', ['id' => ':id']) }}'
                        .replace(':id', id),
                    type: "GET",
                    success: function(data) {
                        $('#id').val(data.id);
                        $('#inverter-name-edit').val(data.inverter_name);
                        $('#Unit-edit').val(data.kw);
                        $('#inverterEditModal').modal('show');
                    },
            ...
            $(document).on('change', '.toggle-status', function() {
                var inverterId = $(this).data("inverter-id");
                var status = $(this).prop('checked') ? 'active' : 'inactive';
                var toggleElement = $(this);

                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.update_inverter_status', ['id' => ':id', 'status' => ':status']) }}'
            ...
            $(document).on('click', '#delete_btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('admin.delete_inverter', ['id' => ':id']) }}'
                                .replace(':id', inverterId),
                            type: 'GET',
                            success: function(response) {
                                // Swal.fire({
