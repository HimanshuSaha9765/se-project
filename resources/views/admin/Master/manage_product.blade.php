    ...
    <div class="x_panel">
        <div class="x_content">
            <div style="display: flex; justify-content: flex-end;">
                ...
                <a href="{{ route('admin.manage_purchase_producs') }}">
            ...
            @include('Modals.Product.edit_ProductModal')
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box table-responsive" style="padding: 1rem;">
                        <table id="StockTable" class="table table-bordered  table-hover text-nowrap text-center"
                            style="width:100%">
                            ...
                            <tbody>
                                @foreach ($Product_data as $Product_datas)
                                    <tr>
                                        <td>
                                            <div>
                                        ...
                                        <td>{{ $Product_datas->product_id ?? '-' }}</td>
                                        <td>{{ $Product_datas->product_name ?? '-' }}</td>
                                        <td>{{ $Product_datas->unit ?? '-' }}</td>
                                        <td>
                                            <div style="display: flex;align-items: center;justify-content: center;">
                                                @if ($Product_datas->status == 'active')
                                                    <input type="checkbox" class="toggle-status" checked
                                                        data-toggle="toggle" data-onlabel="Active"
                                                        data-product-id="{{ $Product_datas->id }}" data-offlabel="Inactive"
                                                        data-onstyle="success" data-offstyle="danger" data-size="sm">
                                                @else
                                                    <input type="checkbox" class="toggle-status" data-toggle="toggle"
                                                        data-onlabel="Active" data-product-id="{{ $Product_datas->id }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '.toggle-status', function() {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.update_product_status', ['id' => ':id', 'status' => ':status']) }}'
            ...
            $(document).on('click', '#delete_btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('admin.delete_product', ['id' => ':id']) }}'
                                .replace(':id', panelId),
                            type: 'GET',
                            success: function(response) {
                                location.reload()
