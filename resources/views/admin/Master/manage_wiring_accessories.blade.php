    ...
    <div class="x_panel">
        <div class="x_content">
            <div style="display: flex; justify-content: flex-end;">
                <button type="button" class="btn btn-outline-dark" data-toggle="modal" data-target="#accessoriesModal"
                    data-whatever="@mdo">
            ...
            {{-- ADD MODAL --}}
            <div class="modal fade" id="accessoriesModal" tabindex="-1" role="dialog"
                aria-labelledby="accessoriesModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="accessoriesTableModalLabel">Add accessories</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form id="AddaccessoriesForm" method="POST" action="{{ route('admin.insert_wiring_accessories') }}">
                            @csrf
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="accessories-name" class="col-form-label required_input">Accessories
                                        Name</label>
                                    <input type="text" class="form-control" id="accessories-name"
                                        placeholder="Enter Accessories Name" name="accessories_name" required>
                                </div>
                                <div class="form-group">
            ...
            {{-- EDIT MODAL --}}
            <div class="modal fade" id="accessoriesEditModal" tabindex="-1" role="dialog"
                aria-labelledby="accessoriesModalEditLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="accessoriesTableModalLabel">Edit accessories</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form id="EditaccessoriesForm" method="POST" action="{{ route('admin.update_wiring_accessories') }}">
                            @csrf
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label for="accessories-name" class="col-form-label required_input">Accessories
                                        Name</label>
                                    <input type="text" class="form-control" id="accessories-name-edit"
                                        placeholder="Enter Accessories Name" name="accessories_name_edit" required>
                                </div>
                                <div class="form-group">
                                    <label for="Unit" class="col-form-label">Unit</label>
            ...
            {{-- INFO MODAL --}}
            <div class="modal fade bs-example-modal-lg" id="accessoriesInfoModal" tabindex="-1" role="dialog"
                aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="accessoriesTableModalLabel">Information</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box table-responsive" style="padding: 1rem;">
                        <table id="accessoriesTable" class="table table-bordered  table-hover text-nowrap text-center"
                            style="width:100%">
                            ...
                            <tbody>
                                @foreach ($wiring_accessories as $accessories)
                                    <tr>
                                        <td>
                                            <div>
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                                    <button class="dropdown-item edit-btn" type="button"
                                                        data-id="{{ $accessories->id }}"> <i class="fa fa-edit mx-2"></i>
                                                        Edit</button>
                                                    <button id="delete_btn" class="dropdown-item" type="button"
                                                        data-id="{{ $accessories->id }}">
                                                        <i class="glyphicon glyphicon-trash mx-2"></i>
                                                        Delete
                                                    </button>
                                                    <button id="info" class="dropdown-item" type="button"
                                                        data-id="{{ $accessories->info_id }}" data-toggle="modal"
                                                        data-target="#accessoriesInfoModal">
                                            ...
                                            <div style="display: flex;align-items: center;justify-content: center;">
                                                ...
                                                @else
                                                    <input type="checkbox" class="toggle-status" data-toggle="toggle"
                                                        data-onlabel="Active" data-accessories-id="{{ $accessories->id }}"
                                                        data-offlabel="Inactive" data-onstyle="success"
                                                        data-offstyle="danger" data-size="sm">
                                                @endif
                                            </div>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            $('#accessoriesTable thead tr')
                .clone(true)
                .addClass('filters')
                .appendTo('#accessoriesTable thead');

            var table = $('#accessoriesTable').DataTable({
                dom: 'Bfrtip',
                stateSave: true,
                ordering: false,
                pageLength: 0,
            ...
            var addForm = $("#AddaccessoriesForm");
            addForm.validate({
                errorElement: 'span',
            ...
            $('.table').on('click', '.edit-btn', function(e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route('admin.edit_wiring_accessories', ['id' => ':id']) }}'
                        .replace(':id', id),
                    type: "GET",
                    success: function(data) {
                        $('#id').val(data.id);
                        $('#accessories-name-edit').val(data.accessories_name);
                        $('#Unit-edit').val(data.unit);
                        $('#accessoriesEditModal').modal('show');
                    },
            ...
            var editForm = $("#EditaccessoriesForm");
            ...
            $(document).on('change', '.toggle-status', function() {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.update_wiring_accessories_status', ['id' => ':id', 'status' => ':status']) }}'
                        .replace(':id', accessoriesId)
            ...
            $(document).on('click', '#delete_btn', function() {
                var accessoriesId = $(this).data("id");
                Swal.fire({
                    title: "Are you sure?",
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('admin.delete_wiring_accessories', ['id' => ':id']) }}'
                                .replace(':id', accessoriesId),
                            type: 'GET',
                            success: function(response) {
                                // Swal.fire({
