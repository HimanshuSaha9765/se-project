    ...
    <div class="x_panel">
        <div class="x_content">
            <div style="display: flex; justify-content: flex-end;">
                <button type="button" class="btn btn-outline-dark" data-toggle="modal" data-target="#cableStockModal"
                    data-whatever="@mdo">
            ...
            <div class="modal fade" id="cableStockModal" tabindex="-1" role="dialog" aria-labelledby="cableStockModalLabel"
                aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="cableStockTableModalLabel">Add Cable Stock</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form id="AddCableStockForm" action="{{ route('admin.insert_cable_stock') }}" method="POST">
                                @csrf
                                <div class="form-group">
                                    <label for="cable-name" class="col-form-label required_input">Cable</label>
                                    <select class="form-control select2" id="cable_info_id" name="cable_info_id"
                                        style="width: 100%" required>
                                        @foreach ($Cables as $cable)
                                            <option value="{{ $cable->info_id }}"
                                                @if (old('cable_info_id') == '{{ $cable->info_id }}') selected @endif>
            ...
            <div class="modal fade" id="cableStockEditModal" tabindex="-1" role="dialog"
                aria-labelledby="cableStockModalEditLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="cableStockTableModalLabel">Edit Cable Stock</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form id="EditCableStockForm" method="POST" action="{{ route('admin.update_cable_stock') }}">
                            @csrf
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label for="cable-name" class="col-form-label required_input">Cable
                                        Name</label>
                                    <h2 id="cable-name-edit"
            ...
            {{-- INFO MODAL --}}
            <div class="modal fade bs-example-modal-lg" id="cableStockInfoModal" tabindex="-1" role="dialog"
                aria-hidden="true">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box table-responsive" style="padding: 1rem;">
                        <table id="cableStockTable" class="table table-bordered  table-hover text-nowrap text-center"
                            style="width:100%">
                            <thead style="background: white;text-align: center; align-items: center;">
                                <tr>
                                    <th class="text-center">Actions</th>
                                    <th class="text-center">Cable Id</th>
                            ...
                            <tbody>
                                @foreach ($Cable_stocks as $cable_stock)
                                    <tr>
                                        <td>
                                            <div>
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                                    <button class="dropdown-item edit-btn" type="button"
                                                        data-id="{{ $cable_stock->id }}"> <i class="fa fa-edit mx-2"></i>
                                                        Edit</button>
                                                    <button id="delete_btn" class="dropdown-item" type="button"
                                                        data-id="{{ $cable_stock->id }}">
                                                        <i class="glyphicon glyphicon-trash mx-2"></i>
                                                        Delete
                                                    </button>
                                                    <button id="info" class="dropdown-item" type="button"
                                                        data-id="{{ $cable_stock->info_id }}" data-toggle="modal"
                                                        data-target="#cableStockInfoModal">
                                        ...
                                        <td>
                                            <div class="row"
                                                style="align-items: center;justify-content: center;gap: 1rem;">
                                                <div>
                                                    {{ $cable_stock->cable->cable_type ?? '-' }}
                                                </div> x
                                                <div>
                                                    {{ $cable_stock->cable->cable_length }}
                                                </div>(
                                                @if ($cable_stock->cable->cable_color === 'red')
                                                    <div style="height: 20px;width: 20px;background-color: red"></div>
                                                @elseif ($cable_stock->cable->cable_color === 'green')
                                                    <div style="height: 20px;width: 20px;background-color: limegreen">
                                                    </div>
                                                @else
                                        ...
                                        <td>{{ $cable_stock->total_qty ?? '-' }}</td>
                                        <td>{{ $cable_stock->date ?? '-' }}</td>
                                        <td>
                                            <div style="display: flex;align-items: center;justify-content: center;">
                                                @if ($cable_stock->status == 'active')
                                                    <input type="checkbox" class="toggle-status" checked
                                                        data-toggle="toggle" data-onlabel="Active"
                                                        data-cable-stock-id="{{ $cable_stock->id }}"
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#cableStockTable thead tr')
                .clone(true)
                .addClass('filters')
                .appendTo('#cableStockTable thead');

            var table = $('#cableStockTable').DataTable({
                dom: 'Bfrtip',
                stateSave: true,
                ordering: false,
                pageLength: 0,
            ...
            $('.table').on('click', '.edit-btn', function(e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route('admin.edit_cable_stock', ['id' => ':id']) }}'
                        .replace(':id', id),
                    type: "GET",
                    success: function(data) {
                        $('#id').val(data.id);
                        $.ajax({
                            url: '{{ route('admin.getCables') }}',
                            type: "GET",
                            success: function(cableData) {
                                var matchingCable = cableData.find(function(
                                    cable) {
                                    return cable.info_id === data.cable_id;
                                });

                                if (matchingCable) {
                                    $('#cable-name-edit').html(
                                        matchingCable.cable_name);
                                } else {
                                    $('#cable-name-edit').html(
                                        "Nothing found");
                                }
                        ...
                        $('#cableStockEditModal').modal('show');
                    },
                    error: function(xhr, status, error) {
                        console.log(xhr.responseText);
                    }
            ...
            var editForm = $("#EditCableStockForm");
            ...
            $(document).on('change', '.toggle-status', function() {
                var cableId = $(this).data("cable-stock-id");
                var status = $(this).prop('checked') ? 'active' : 'inactive';
                var toggleElement = $(this);

                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.update_cable_stock_status', ['id' => ':id', 'status' => ':status']) }}'
                        .replace(':id', cableId)
            ...
            $(document).on('click', '#delete_btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('admin.delete_cable_stock', ['id' => ':id']) }}'
                                .replace(':id', panelId),
                            type: 'GET',
                            success: function(response) {
                                // Swal.fire({
