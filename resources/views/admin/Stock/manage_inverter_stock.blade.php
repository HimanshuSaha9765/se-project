    ...
    <div class="x_panel">
        <div class="x_content">
            <div style="display: flex; justify-content: flex-end;">
                <button type="button" class="btn btn-outline-dark" data-toggle="modal" data-target="#inverterStockModal"
                    data-whatever="@mdo">
            ...
            <div class="modal fade" id="inverterStockModal" tabindex="-1" role="dialog"
                aria-labelledby="inverterStockModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="inverterStockTableModalLabel">Add Inverter Stock</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form id="AddInverterStockForm" action="{{ route('admin.insert_inverter_stock') }}"
                                method="POST">
                                @csrf
                                <div class="form-group">
                                    <label for="inverter-name" class="col-form-label required_input">inverter name</label>
                                    <select class="form-control select2" id="inverter_info_id" name="inverter_info_id"
                                        style="width: 100%" required>
                                        @foreach ($Inverters as $inverter)
                                            <option value="{{ $inverter->info_id }}"
                                                @if (old('inverter_info_id') == '{{ $inverter->info_id }}') selected @endif>
            ...
            <div class="modal fade" id="inverterStockEditModal" tabindex="-1" role="dialog"
                aria-labelledby="inverterStockModalEditLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="inverterStockTableModalLabel">Edit Inverter Stock</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form id="EditInverterStockForm" method="POST" action="{{ route('admin.update_inverter_stock') }}">
                            @csrf
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label for="inverter-name" class="col-form-label required_input">Inverter
                                        Name</label>
                                    <h2 id="inverter-name-edit"
            ...
            {{-- INFO MODAL --}}
            <div class="modal fade bs-example-modal-lg" id="inverterStockInfoModal" tabindex="-1" role="dialog"
                aria-hidden="true">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box table-responsive" style="padding: 1rem;">
                        <table id="inverterStockTable" class="table table-bordered  table-hover text-nowrap text-center"
                            style="width:100%">
                            <thead style="background: white;text-align: center; align-items: center;">
                                <tr>
                                    <th class="text-center">Actions</th>
                                    <th class="text-center">Inverter Id</th>
                            ...
                            <tbody>
                                @foreach ($Inverter_stocks as $inverter_stock)
                                    <tr>
                                        <td>
                                            <div>
                                        ...
                                        <td>{{ $inverter_stock->inverter->inverter_name ?? '-' }}
                                            {{ $inverter_stock->inverter->inverter_brand }}
                                            {{ $inverter_stock->inverter->kw }}
                                        </td>
                                        <td>{{ $inverter_stock->total_qty ?? '-' }}</td>
                                        <td>{{ $inverter_stock->date ?? '-' }}</td>
                                        <td>
                                            <div style="display: flex;align-items: center;justify-content: center;">
                                                @if ($inverter_stock->status == 'active')
                                                    <input type="checkbox" class="toggle-status" checked
                                                        data-toggle="toggle" data-onlabel="Active"
                                                        data-inverter-stock-id="{{ $inverter_stock->id }}"
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#inverterStockTable thead tr')
                .clone(true)
                .addClass('filters')
                .appendTo('#inverterStockTable thead');

            var table = $('#inverterStockTable').DataTable({
                dom: 'Bfrtip',
                stateSave: true,
                ordering: false,
                pageLength: 0,
            ...
            var addForm = $("#AddInverterStockForm");
            addForm.validate({
                rules: {
                    total_qty: "digits"
                },
            ...
            $('.table').on('click', '.edit-btn', function(e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route('admin.edit_inverter_stock', ['id' => ':id']) }}'
                        .replace(':id', id),
                    type: "GET",
                    success: function(data) {
                        $('#id').val(data.id);
                        $.ajax({
                            url: '{{ route('admin.getInverters') }}',
                            type: "GET",
                            success: function(inverterData) {
                                var matchingInverter = inverterData.find(function(
                                    inverter) {
                                    return inverter.info_id === data
                                        .inverter_id;
                                });

                                if (matchingInverter) {
                                    $('#inverter-name-edit').html(
                                        matchingInverter.inverter_name);
                                } else {
                                    $('#inverter-name-edit').html(
                                        "Nothing found");
                                }
                        ...
                        $('#inverterStockEditModal').modal('show');
                    },
                    error: function(xhr, status, error) {
                        console.log(xhr.responseText);
                    }
            ...
            var editForm = $("#EditInverterStockForm");
            ...
            $(document).on('change', '.toggle-status', function() {
                var inverterId = $(this).data("inverter-stock-id");
                var status = $(this).prop('checked') ? 'active' : 'inactive';
                var toggleElement = $(this);

                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.update_inverter_stock_status', ['id' => ':id', 'status' => ':status']) }}'
                        .replace(':id', inverterId)
            ...
            $(document).on('click', '#delete_btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('admin.delete_inverter_stock', ['id' => ':id']) }}'
                                .replace(':id', inverterId),
                            type: 'GET',
                            success: function(response) {
                                // Swal.fire({
