    ...
    <div class="x_panel">
        <div class="x_content">
            <div style="display: flex; justify-content: flex-end;">
                <button type="button" class="btn btn-outline-dark" data-toggle="modal" data-target="#PanelStockModal"
                    data-whatever="@mdo">
            ...
            <div class="modal fade" id="PanelStockModal" tabindex="-1" role="dialog" aria-labelledby="PanelStockModalLabel"
                aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="panelStockTableModalLabel">Add Panel Stock</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form id="AddPanelStockForm" action="{{ route('admin.insert_panel_stock') }}" method="POST">
                                @csrf
                                <div class="form-group">
                                    <label for="accessories-name" class="col-form-label required_input">Accessories
                                        Name</label>
                                    <select class="form-control select2" id="panel_info_id" name="panel_info_id"
                                        style="width: 100%" required>
                                        @foreach ($Panels as $panel)
                                            <option value="{{ $panel->info_id }}"
                                                @if (old('panel_info_id') == '{{ $panel->info_id }}') selected @endif>
            ...
            <div class="modal fade" id="panelStockEditModal" tabindex="-1" role="dialog"
                aria-labelledby="panelStockModalEditLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="panelStockTableModalLabel">Edit Panel Stock</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form id="EditPanelStockForm" method="POST" action="{{ route('admin.update_panel_stock') }}">
                            @csrf
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label for="panel-name" class="col-form-label required_input">Panel
                                        Name</label>
                                    <h2 id="panel-name-edit"
            ...
            {{-- INFO MODAL --}}
            <div class="modal fade bs-example-modal-lg" id="panelStockInfoModal" tabindex="-1" role="dialog"
                aria-hidden="true">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box table-responsive" style="padding: 1rem;">
                        <table id="panelStockTable" class="table table-bordered  table-hover text-nowrap text-center"
                            style="width:100%">
                            <thead style="background: white;text-align: center; align-items: center;">
                                <tr>
                                    <th class="text-center">Actions</th>
                                    <th class="text-center">Panel Id</th>
                            ...
                            <tbody>
                                @foreach ($Panel_stocks as $panel_stock)
                                    <tr>
                                        <td>
                                            <div>
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                                    <button class="dropdown-item edit-btn" type="button"
                                                        data-id="{{ $panel_stock->id }}"> <i class="fa fa-edit mx-2"></i>
                                                        Edit</button>
                                                    <button id="delete_btn" class="dropdown-item" type="button"
                                                        data-id="{{ $panel_stock->id }}">
                                                        <i class="glyphicon glyphicon-trash mx-2"></i>
                                                        Delete
                                                    </button>
                                                    <button id="info" class="dropdown-item" type="button"
                                                        data-id="{{ $panel_stock->info_id }}" data-toggle="modal"
                                                        data-target="#panelStockInfoModal">
                                        ...
                                        <td>{{ $panel_stock->panel->panel_name ?? '-' }}
                                            {{ $panel_stock->panel->category }}</td>
                                        <td>{{ $panel_stock->total_qty ?? '-' }}</td>
                                        <td>{{ $panel_stock->date ?? '-' }}</td>
                                        <td>
                                            <div style="display: flex;align-items: center;justify-content: center;">
                                                @if ($panel_stock->status == 'active')
                                                    <input type="checkbox" class="toggle-status" checked
                                                        data-toggle="toggle" data-onlabel="Active"
                                                        data-panel-stock-id="{{ $panel_stock->id }}"
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#panelStockTable thead tr')
                .clone(true)
                .addClass('filters')
                .appendTo('#panelStockTable thead');

            var table = $('#panelStockTable').DataTable({
                dom: 'Bfrtip',
                stateSave: true,
                ordering: false,
                pageLength: 0,
            ...
            $('.table').on('click', '.edit-btn', function(e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route('admin.edit_panel_stock', ['id' => ':id']) }}'
                        .replace(':id', id),
                    type: "GET",
                    success: function(data) {
                        $('#id').val(data.id);
                        $.ajax({
                            url: '{{ route('admin.getPanels') }}',
                            type: "GET",
                            success: function(panelData) {
                                var matchingPanel = panelData.find(function(
                                    panel) {
                                    return panel.info_id === data.panel_id;
                                });

                                if (matchingPanel) {
                                    $('#panel-name-edit').html(
                                        matchingPanel.panel_name);
                                } else {
                                    $('#panel-name-edit').html(
                                        "Nothing found");
                                }
                        ...
                        $('#panelStockEditModal').modal('show');
                    },
                    error: function(xhr, status, error) {
                        console.log(xhr.responseText);
                    }
            ...
            var editForm = $("#EditPanelStockForm");
            ...
            $(document).on('change', '.toggle-status', function() {
                var panelId = $(this).data("panel-stock-id");
                var status = $(this).prop('checked') ? 'active' : 'inactive';
                var toggleElement = $(this);

                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.update_panel_stock_status', ['id' => ':id', 'status' => ':status']) }}'
                        .replace(':id', panelId)
            ...
            $(document).on('click', '#delete_btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('admin.delete_panel_stock', ['id' => ':id']) }}'
                                .replace(':id', panelId),
                            type: 'GET',
                            success: function(response) {
                                // Swal.fire({
