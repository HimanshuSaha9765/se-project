    ...
    <div class="x_panel">
        <div class="x_content">
            <div style="display: flex; justify-content: flex-end;">
                <button type="button" class="btn btn-outline-dark" data-toggle="modal" data-target="#wiringStockModal"
                    data-whatever="@mdo">
            ...
            <div class="modal fade" id="wiringStockModal" tabindex="-1" role="dialog"
                aria-labelledby="wiringStockModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="wiringStockTableModalLabel">Add Wiring Stock</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form id="AddWiringStockForm" action="{{ route('admin.insert_wiring_stock') }}" method="POST">
                                @csrf
                                <div class="form-group">
                                    <label for="accessories-name" class="col-form-label required_input">Accessories
                                        Name</label>
                                    <select class="form-control select2" id="wiring_info_id" name="wiring_info_id"
                                        style="width: 100%" required>
                                        @foreach ($wirings as $wiring)
                                            <option value="{{ $wiring->info_id }}"
                                                @if (old('wiring_info_id') == '{{ $wiring->info_id }}') selected @endif>
            ...
            <div class="modal fade" id="wiringStockEditModal" tabindex="-1" role="dialog"
                aria-labelledby="wiringStockModalEditLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="wiringStockTableModalLabel">Edit Wiring Stock</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form id="EditWiringStockForm" method="POST" action="{{ route('admin.update_wiring_stock') }}">
                            @csrf
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label for="wiring-name" class="col-form-label required_input">Wiring
                                        Name</label>
                                    <h2 id="wiring-name-edit"
            ...
            {{-- INFO MODAL --}}
            <div class="modal fade bs-example-modal-lg" id="wiringStockInfoModal" tabindex="-1" role="dialog"
                aria-hidden="true">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box table-responsive" style="padding: 1rem;">
                        <table id="wiringStockTable" class="table table-bordered  table-hover text-nowrap text-center"
                            style="width:100%">
                            <thead style="background: white;text-align: center; align-items: center;">
                                <tr>
                                    <th class="text-center">Actions</th>
                                    <th class="text-center">Wiring Id</th>
                            ...
                            <tbody>
                                @foreach ($wiring_stocks as $wiring_stock)
                                    <tr>
                                        <td>
                                            <div>
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                                    <button class="dropdown-item edit-btn" type="button"
                                                        data-id="{{ $wiring_stock->id }}"> <i class="fa fa-edit mx-2"></i>
                                                        Edit</button>
                                                    <button id="delete_btn" class="dropdown-item" type="button"
                                                        data-id="{{ $wiring_stock->id }}">
                                                        <i class="glyphicon glyphicon-trash mx-2"></i>
                                                        Delete
                                                    </button>
                                                    <button id="info" class="dropdown-item" type="button"
                                                        data-id="{{ $wiring_stock->info_id }}" data-toggle="modal"
                                                        data-target="#wiringStockInfoModal">
                                        ...
                                        <td>{{ $wiring_stock->wiring->accessories_name ?? '-' }}
                                            {{ $wiring_stock->wiring->unit }}</td>
                                        <td>{{ $wiring_stock->total_qty ?? '-' }}</td>
                                        <td>{{ $wiring_stock->date ?? '-' }}</td>
                                        <td>
                                            <div style="display: flex;align-items: center;justify-content: center;">
                                                @if ($wiring_stock->status == 'active')
                                                    <input type="checkbox" class="toggle-status" checked
                                                        data-toggle="toggle" data-onlabel="Active"
                                                        data-wiring-stock-id="{{ $wiring_stock->id }}"
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#wiringStockTable thead tr')
                .clone(true)
                .addClass('filters')
                .appendTo('#wiringStockTable thead');

            var table = $('#wiringStockTable').DataTable({
                dom: 'Bfrtip',
                stateSave: true,
                ordering: false,
                pageLength: 0,
            ...
            var addForm = $("#AddWiringStockForm");
            addForm.validate({
                rules: {
                    total_qty: "digits"
                },
            ...
            $('.table').on('click', '.edit-btn', function(e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route('admin.edit_wiring_stock', ['id' => ':id']) }}'
                        .replace(':id', id),
                    type: "GET",
                    success: function(data) {
                        $('#id').val(data.id);
                        $.ajax({
                            url: '{{ route('admin.getWirings') }}',
                            type: "GET",
                            success: function(wiringData) {
                                var matchingWiring = wiringData.find(function(
                                    wiring) {
                                    return wiring.info_id === data
                                        .wiring_id;
                                });

                                if (matchingWiring) {
                                    $('#wiring-name-edit').html(
                                        matchingWiring.accessories_name);
                                } else {
                                    $('#wiring-name-edit').html(
                                        "Nothing found");
                                }
                        ...
                        $('#wiringStockEditModal').modal('show');
                    },
                    error: function(xhr, status, error) {
                        console.log(xhr.responseText);
                    }
            ...
            var editForm = $("#EditWiringStockForm");
            ...
            $(document).on('change', '.toggle-status', function() {
                var wiringId = $(this).data("wiring-stock-id");
                var status = $(this).prop('checked') ? 'active' : 'inactive';
                var toggleElement = $(this);

                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.update_wiring_stock_status', ['id' => ':id', 'status' => ':status']) }}'
                        .replace(':id', wiringId)
            ...
            $(document).on('click', '#delete_btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('admin.delete_wiring_stock', ['id' => ':id']) }}'
                                .replace(':id', panelId),
                            type: 'GET',
                            success: function(response) {
                                // Swal.fire({
